...
namespace Divena_CMS.Controllers
{
    ...
    public class PageController : Controller
    {
        ...
        public IActionResult Add(int? id)
        {
            ...
            if (id != null)
            {
                using (var context = new Divena_CMSContext())
                {
                    ...
                    var virtualpage = context.Page.FromSqlRaw("SELECT * FROM Page WHERE id=" + id).SingleOrDefault();
                    page = virtualpage;
                    ...
                    if (id == null)
                    {
                        var param = new SqlParameter[] {
                                    ...
                                    new SqlParameter() {
                                        ...
                                        Direction = System.Data.ParameterDirection.Input,
                                        Size=-1,
                                        Value = page.Description ?? (object)DBNull.Value
                                    },
                                    new SqlParameter() {
        ...
        public async Task<IActionResult> Delete(int id)
        {
            using (var context = new Divena_CMSContext())
            {
                Page page = await context.Page.Where(p => p.Id == id).FirstOrDefaultAsync();
                context.Remove(page);
