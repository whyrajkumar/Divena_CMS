...
namespace Divena_CMS.Controllers
{
    ...
    {
        private readonly Divena_CMSContext context;

        public BlogCategoryController(Divena_CMSContext context)
        {
            context = context;
        }
        ...
        public async Task<IActionResult> Delete(int id)
        {
            using (var context = new Divena_CMSContext())
            {
                BlogCategory blogCategory = await context.BlogCategory.Where(p => p.Id == id).FirstOrDefaultAsync();
                context.Remove(blogCategory);
